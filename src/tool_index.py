...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "mkdir": "Create a directory inside the workspace (like mkdir -p), including parent directories. Prefer over bash mkdir — confined to the workspace.",
    "rm": "Remove a file or directory inside the workspace (like rm, or rm -r with recursive=True). Prefer over bash rm — confined to the workspace.",
    "mv": "Move or rename a file or directory within the workspace; both paths must be inside it. Prefer over bash mv — confined to the workspace.",
    "cp": "Copy a file or directory within the workspace (like cp, or cp -r with recursive=True). Prefer over bash cp — confined to the workspace.",
