...
Slash commands are typed directly in the Odysseus chat input. Type `/` to open the autocomplete
popup — it shows a curated list of commonly-used commands. Keep typing to filter, use arrow keys
to navigate, and press Tab or Enter to insert. Subcommands (e.g. `/homelab disk`) expand when
you complete the parent token.

Commands are defined in `static/js/slashCommands.js`; the autocomplete popup lives in
...
Requires the OpenClaw bridge to be reachable (`OPENCLAW_ODYSSEUS_URL` + token). All endpoints
are under `/api/openclaw/homelab/`.

| Command | Aliases | What it does | Endpoint |
|---------|---------|--------------|----------|
| `/homelab health` | | Bridge health check | `GET /health` |
| `/homelab converge` | | Converge/Redmine connectivity check | `GET /api/openclaw/tickets/digest` |
...
Reads Redmine/Converge ticket data via `CONVERGE_BASE_URL` + `CONVERGE_API_KEY`. Endpoints are
under `/api/openclaw/`.
...
Reads n8n via `N8N_BASE_URL` + `N8N_API_KEY`. Write operations (`trigger`, `pause`) require the
workflow name to appear in `N8N_WORKFLOW_ALLOWLIST` and display a confirmation prompt. Endpoints
are under `/api/openclaw/n8n/`.
...
Shows all connected MCP servers — both built-in servers and any user-added servers. Calls
`GET /api/mcp/servers`. Built-in servers include `image_gen`, `memory`, `rag`, `email`,
and `prometheus` (Python stdio) plus `builtin_browser` (npx, if cached). See the
[README MCP section](../README.md#built-in-mcp-servers-optional-setup) for setup details.
