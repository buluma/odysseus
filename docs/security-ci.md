...
4. Under **Branch protection rules**, click **Add branch ruleset** (or **Add rule**), and set the branch name pattern to `local` (this is the repo's default branch and the one pull requests target — `dev` and `main` are both stale, not what PRs merge into).
...
`.github/dependabot.yml` opens small weekly pull requests to update Python and bun packages, the Docker base image, and the pinned automation actions themselves. Review and merge those like any other pull request; they keep the project patched without manual tracking.
