...
### Deduplication
When `record_events=true` is used against the health API, Odysseus uses a stable `dedupe_key` to intelligently map repeated failures to an open event rather than spamming new incidents. Example deduplication keys:
...
### 1. List All Events
**Route:** `GET /api/events` **Requires:** `events:read` **Query Parameters:**
...
**Route:** `GET /api/events/summary` **Requires:** `events:read` Returns a compact summary of the top 10 open events, tailored for conversational UIs.

### 3. Get Specific Event
**Route:** `GET /api/events/{id}` **Requires:** `events:read`

### 4. Acknowledge Event
**Route:** `POST /api/events/{id}/ack` **Requires:** `events:ack`

### 5. Investigate Event
**Route:** `POST /api/events/{id}/investigate` **Requires:** `events:ack`

### 6. Resolve Event
**Route:** `POST /api/events/{id}/resolve` **Requires:** `events:resolve`

### 7. Ignore Event
**Route:** `POST /api/events/{id}/ignore` **Requires:** `events:resolve`
